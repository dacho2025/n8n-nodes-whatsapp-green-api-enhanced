...
export class GreenApiGetImage implements INodeType {
	description: INodeTypeDescription = {
		...
		group: ['communication'],
		version: 1,
		description: 'Download and process WhatsApp image messages via Green API',
		defaults: {
		...
		properties: [
			{
				...
				options: [
					{
						name: 'Message ID',
						value: 'messageId',
						description: 'Use message ID to download image',
					},
					{
						name: 'Download URL',
						value: 'downloadUrl',
				...
				description: 'Source of the image message data',
			},
			{
				...
				default: '',
				description: 'WhatsApp message ID containing the image',
			},
			{
				displayName: 'Chat ID',
				name: 'chatId',
			...
			},
			{
				...
				default: '',
				description: 'Direct download URL for the image',
			},
			{
			...
			},
			{
				...
				options: [
					...
					},
					{
						...
						default: [],
						description: 'Only process images of selected formats (leave empty for all formats)',
					},
					{
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (downloadUrl) {
					...
					imageData = Buffer.from(response.body);
					fileName = fileInfoResponse.fileName || 'image.jpg';
					mimeType = fileInfoResponse.mimeType || 'image/jpeg';
				...
				if (options.maxFileSize && options.maxFileSize > 0) {
					...
					if (imageData.length > maxSizeBytes) {
						throw new NodeOperationError(
							this.getNode(),
							`File size (${(imageData.length / 1024 / 1024).toFixed(2)}MB) exceeds limit (${options.maxFileSize}MB)`,
							{ itemIndex: i },
						);
					}
				}
				...
				if (options.formatFilter && options.formatFilter.length > 0) {
					...
					for (const filterFormat of options.formatFilter) {
						if (formatMap[filterFormat]?.includes(fileExtension)) {
							formatMatches = true;
							break;
						}
					}
