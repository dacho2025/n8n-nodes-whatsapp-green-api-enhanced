	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			...
			try {
				...
				switch (resource) {
					case 'message':
						responseData = await MessageOperations.execute(
							this,
							i,
							operation,
							instanceId,
					...
					case 'group':
						responseData = await GroupOperations.execute(
							this,
							i,
							operation,
							instanceId,
					...
					case 'chat':
						responseData = await ChatOperations.execute(
							this,
							i,
							operation,
							instanceId,
					...
					case 'contact':
						responseData = await ContactOperations.execute(
							this,
							i,
							operation,
							instanceId,
				...
				throw new NodeOperationError(
					this.getNode(),
					`Error in ${resource} ${operation}: ${(error as Error).message}`,
					{ itemIndex: i }
				);
			}
		}
